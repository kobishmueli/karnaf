...
if(isset($_POST['save']) && ($_POST['save'] == "2")) {
  ...
  if(!empty($_POST['merged_to']) && $result['merged_to'] != $_POST['merged_to']) {
    ...
    if(($result2 = sql_fetch_array($query2))) {
      ...
      squery("INSERT INTO karnaf_replies(tid,title,reply,r_time,r_from,r_by,ip) VALUES(%d,'%s','%s',%d,'%s','%s','%s')",
...
function submit3_onclick() {
  ...
  document.form1.submit();
    ...
    function autocomplate_me(e) {
        ...
        $(e.self.event.target).on("keydown", function (event) {
            if (event.keyCode === $.ui.keyCode.TAB &&
                $(e.self.event.target).autocomplete("instance").menu.active) {
                event.preventDefault();
            }
        })
            .autocomplete({
                source: function (request, response) {
                    $.getJSON("ajax_searchemail.php", {
